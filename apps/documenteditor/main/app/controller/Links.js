    ...
    _.extend(
      {
        ...
        setApi: function (api) {
          if (api) {
            ...
            this.api.asc_registerCallback(
              "asc_onFocusObject",
              this.onApiFocusObject.bind(this)
            );
            this.api.asc_registerCallback(
            ...
            this.api.asc_registerCallback(
              "asc_onAscTOFUpdate",
              _.bind(this.onAscTOFUpdate, this)
            );
          }
          Common.NotificationCenter.on(
            "protect:doclock",
        ...
        onApiFocusObject: function (selectedObjects) {
          ...
          while (++i < selectedObjects.length) {
            ...
            if (type === Asc.c_oAscTypeSelectElement.Paragraph) {
              ...
              in_image = true;
              in_image_inline =
                pr.get_WrappingStyle() === Asc.c_oAscWrapStyle2.Inline;
              object_type = type;
          ...
          var control_props = this.api.asc_IsContentControl()
              ? this.api.asc_GetContentControlProperties()
              : null,
            control_plain = control_props
              ? control_props.get_ContentControlType() ==
                Asc.c_oAscSdtLevelType.Inline
              : false,
            lock_type = control_props
          ...
          var rich_del_lock = frame_pr
              ? !frame_pr.can_DeleteBlockContentControl()
              : false,
            rich_edit_lock = frame_pr
              ? !frame_pr.can_EditBlockContentControl()
              : false,
            plain_del_lock = frame_pr
              ? !frame_pr.can_DeleteInlineContentControl()
              : false,
            plain_edit_lock = frame_pr
              ? !frame_pr.can_EditInlineContentControl()
              : false;
          ...
          this.lockToolbar(Common.enumLock.controlPlain, control_plain, {
            array: this.view.btnsNotes.concat([
              this.view.btnBookmarks,
              this.view.btnCrossRef,
            ]),
          });
          ...
          this.lockToolbar(Common.enumLock.headerLock, header_locked, {
            array: this.view.btnsHyperlink.concat([
              this.view.btnBookmarks,
              this.view.btnCrossRef,
            ]),
          });
          this.lockToolbar(Common.enumLock.inEquation, in_equation, {
            array: this.view.btnsNotes,
          });
          this.lockToolbar(Common.enumLock.inImage, in_image, {
            array: this.view.btnsNotes,
          });
          ...
          this.lockToolbar(
            ...
            this.api.asc_IsCursorInFootnote() ||
              this.api.asc_IsCursorInEndnote(),
          ...
          this.dlgCrossRefDialog &&
            this.dlgCrossRefDialog.isVisible() &&
            this.dlgCrossRefDialog.setLocked(
              this.view.btnCrossRef.isDisabled()
            );
        },
            ...
            if (text !== false) {
              ...
              if (selectedElements && _.isArray(selectedElements)) {
                _.each(selectedElements, function (el, i) {
                  if (
                    selectedElements[i].get_ObjectType() ==
        ...
        onTableContents: function (type, currentTOC) {
          ...
          switch (type) {
              ...
              var me = this,
                win = new DE.Views.TableOfContentsSettings({
                  ...
                  handler: function (result, value) {
                    if (result == "ok") {
                      props
                        ? me.api.asc_SetTableOfContentsPr(value)
                        : me.api.asc_AddTableOfContents(null, value);
                    }
            ...
            case "remove":
              currentTOC =
                currentTOC && props ? props.get_InternalClass() : undefined;
        ...
        onAddTextOpen: function (menu) {
          ...
          this.view.fillAddTextMenu(
            menu,
            end,
            this.api.asc_GetCurrentLevelTOC()
          );
        },
            ...
            case "settings":
              ...
              new DE.Views.NoteSettingsDialog({
                ...
                handler: function (result, settings) {
                  if (settings) {
                    settings.isEndNote
                      ? me.api.asc_SetEndnoteProps(
                          settings.props,
                          settings.applyToAll
                        )
                      : me.api.asc_SetFootnoteProps(
                          settings.props,
                          settings.applyToAll
                        );
                    if (result == "insert")
                      setTimeout(function () {
                        settings.isEndNote
                          ? me.api.asc_AddEndnote(settings.custom)
                ...
                props: isEndNote
                  ? me.api.asc_GetEndnoteProps()
                  : me.api.asc_GetFootnoteProps(),
              }).show();
              break;
            case "prev":
        ...
        onShowTOCActions: function (obj, x, y) {
          var action = obj.button,
            ...
            documentHolder =
              this.getApplication().getController("DocumentHolder"),
            documentHolderView = documentHolder.getView(),
            menuContainer = documentHolderView.cmpEl.find(
        ...
        onShowContentControlsActions: function (obj, x, y) {
          obj.type == Asc.c_oAscContentControlSpecificType.TOC &&
            this.onShowTOCActions(obj, x, y);
        },

        onCrossRefClick: function (btn) {
          if (this.dlgCrossRefDialog && this.dlgCrossRefDialog.isVisible())
            return;
        ...
        onAscTOFUpdate: function (apiCallback) {
          ...
          new Common.Views.OptionsDialog({
            ...
            items: [
              {
                caption: this.view.textUpdatePages,
                value: true,
                checked: true,
              },
              {
                caption: this.view.textUpdateAll,
                value: false,
                checked: false,
              },
            ],
        ...
        onChangeProtectDocument: function (props) {
          if (!props) {
            var docprotect =
              this.getApplication().getController("DocProtection");
