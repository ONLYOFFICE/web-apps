...
define([
      ...
      return {
          ...
          if (!me.rendered) {
            ...
            if (me.options.hint) {
              ...
              if (modalParents.length > 0) {
                el.data("bs.tooltip")
                  .tip()
                  .css("z-index", parseInt(modalParents.css("z-index")) + 10);
                var onModalClose = function (dlg) {
                  if (modalParents[0] !== dlg.$window[0]) return;
                  var tip = el.data("bs.tooltip");
                  if (tip) {
                    if (tip.dontShow === undefined) tip.dontShow = true;
              ...
              el.find(".dropdown-menu").on("mouseenter", function () {
            ...
            el.on(
              ...
              _.bind(me.onAfterKeydownMenu, me)
            );

            Common.NotificationCenter.on(
              "menumanager:hideall",
              _.bind(me.closeMenu, me)
            );
        ...
        onBeforeShowMenu: function (e) {
          ...
          if ($list.hasClass("menu-absolute")) {
            ...
            $list.css({
              left: left,
              top: offset.top + this.cmpEl.outerHeight() + 2,
            });
          } else if ($list.hasClass("menu-aligned")) {
            var offset = this.cmpEl.offset();
            $list.toggleClass(
              "show-top",
              offset.top + this.cmpEl.outerHeight() + $list.outerHeight() >
        ...
        onAfterShowMenu: function (e) {
          ...
          if ($selected.length) {
            ...
            if (itemTop < 0 || itemTop + itemHeight > listHeight) {
              var height =
                $list.scrollTop() + itemTop + (itemHeight - listHeight) / 2;
        ...
        alignMenuPosition: function () {
          if (this.restoreMenuHeight) {
            ...
            if (menuH < this.restoreMenuHeight) newH = this.restoreMenuHeight;

            if (menuTop + newH > docH) newH = docH - menuTop;

            if (newH !== menuH) $list.css("max-height", newH + "px");
          }
        },
        ...
        onBeforeHideMenu: function (e) {
          ...
          if (Common.UI.Scroller.isMouseCapture()) e.preventDefault();
        },
        ...
        onAfterHideMenu: function (e, isFromInputControl) {
          ...
          if (this.options.takeFocusOnClose) {
            ...
            me._input &&
              me._input.length > 0 &&
              !me.editable &&
              me._input[0].selectionStart === me._input[0].selectionEnd &&
              setTimeout(function () {
        ...
        onAfterKeydownMenu: function (e) {
          if (
            ...
            !this.focusWhenNoSelection &&
            (e.keyCode == Common.UI.Keys.DOWN || e.keyCode == Common.UI.Keys.UP)
          ) {
            var $items = this.cmpEl.find("ul > li a");
            if ($items.filter(":focus").length === 0 && $items.length > 0) {
              setTimeout(function () {
                $items[
                  e.keyCode == Common.UI.Keys.DOWN ? 0 : $items.length - 1
                ].focus();
              }, 1);
            }
          } else if (
            e.keyCode == Common.UI.Keys.RETURN &&
            (this.editable || this.isMenuOpen())
          ) {
        ...
        selectCandidate: function () {
          var index =
              this._search.index && this._search.index != -1
                ? this._search.index
                : 0,
            re = new RegExp(
              "^" + (this._search.full ? this._search.text : this._search.char),
              "i"
            ),
            isFirstCharsEqual = re.test(
              this.store.at(index).get(this.displayField)
            ),
            itemCandidate,
            idxCandidate;
        ...
        onInputKeyDown: function (e) {
          ...
          if (e.keyCode == Common.UI.Keys.ESC) {
            ...
            e.keyCode == Common.UI.Keys.RETURN &&
            $(e.target).val() === me.lastValue
          ) {
            this._input.trigger("change", { reapply: true });
          } else me._skipInputChange = false;
        },
        ...
        onInputChanged: function (e, extra) {
          ...
          if (this.lastValue === val && !(extra && extra.reapply)) {
            if (extra && extra.onkeydown) this.trigger("combo:blur", this, e);
            return;
          }
          ...
          this._selectedItem = this.store.findWhere(
            ((obj = {}), (obj[this.displayField] = val), obj)
          );

          if (this._selectedItem) {
            record = this._selectedItem.toJSON();
            $(".selected", $(this.el)).removeClass("selected");
            $("#" + this._selectedItem.get("id"), $(this.el)).addClass(
              "selected"
            );
          }
        ...
        onInputClick: function (e) {
          if (this._button) this._button.dropdown("toggle");
        ...
        onEditableInputClick: function (e) {
          ...
          if (this.isMenuOpen() && e.which == 1) e.stopPropagation();
        },
        ...
        setValue: function (value, defValue) {
          ...
          var obj;
          this._selectedItem = this.store.findWhere(
            ((obj = {}), (obj[this.valueField] = value), obj)
          );
        ...
        getValue: function () {
          ...
          if (
            this._selectedItem &&
            !_.isUndefined(this._selectedItem.get(this.valueField))
          ) {
        ...
        selectRecord: function (record) {
          if (!this.rendered || !record) return;
        ...
        itemClicked: function (e) {
          ...
          if (this._selectedItem) {
            ...
            this.trigger(
              "selected",
              this,
              _.extend({}, this._selectedItem.toJSON()),
              e
            );
  ...
  Common.UI.ComboBoxCustom = Common.UI.ComboBox.extend(
    _.extend(
      {
        itemClicked: function (e) {
        ...
        setValue: function (value, defValue) {
          ...
          if (this.options.updateFormControl)
            this.options.updateFormControl.call(
              this,
              this._selectedItem,
              defValue
            );
